...
function App() {
  ...
  useEffect(() => {
    if(currentTodo){
      ...
      }).then((res) => res.json()).then((res) => setTodo(prev => prev.map((el) => (el.id === currentTodo ? res : el))))
    }
  }, [time])
  ...
  return (
    <div className='layout-container'>
      <main className='main-content'>
        ...
        <Advice />
        <Clock />
        <button className="timer-change" onClick={() => setIsTimer((prev) => !prev)}>{isTimer ? "스탑워치로 변경": "타이머로 변경"}</button>
